...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell.tag == 111) { // 点击的是父选项cell
        ...
        if ([self.dk_dataSorce containsObject:optionArray[0]]) { // 如果菜单已经展开,关闭菜单
            ...
            // 把icon换成向右的
            cell.accessoryView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"ic_forward"]];
            ...
            // tableView删除行
            [tableView deleteRowsAtIndexPaths:indexPathsArr withRowAnimation:UITableViewRowAnimationRight];
            ...
            // 把icon换成向下的
            cell.accessoryView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"ic_drop_client"]];
            ...
            // tableView插入行
            [tableView insertRowsAtIndexPaths:indexPathsArr withRowAnimation:UITableViewRowAnimationLeft];
        }
...
- (UIImageView *)setMenuAccessoryView:(UITableViewCell *)cell
{
    ...
    if ([self.dk_dataSorce containsObject:optionArr[0]]) { // 如果这个父选项的菜单已经展开了
        return [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"ic_drop_client"]];
    }
    
    return [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"ic_forward"]];
}
